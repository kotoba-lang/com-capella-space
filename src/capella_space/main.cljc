(ns capella-space.main
  "Kotodama WASM entrypoint for the capella_space clean-room actor (L4) — Clojure port.
  ...
  facts (`capella_space.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "capella_space")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"capella_space.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "capella_space-compat" :tier tier :entities entities} 200])
...
(defn start! [] :capella_space-compat/ready)
